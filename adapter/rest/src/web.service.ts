...
export class WebService {
  ...
  listen(port: number = 8123): Observable<any> {
    ...
    io.on('connection', socket => {
      this.$log.info('Connection received')
      ...
      mqtt.on('connect', () => {
        socket.on('subscribe', (data: string | string[]) => {
          this.$log.info('Subscribing', data)
          mqtt.subscribe(data)
        })
        socket.on('unsubscribe', (data: string | string[]) => {
          this.$log.info('Unsubscribe', data)
      ...
      if (token) {
        ...
        this.$users
          ...
          .then((tokenData: Ha4usToken) => {
            ...
            req.admin =
              tokenData.user.roles && tokenData.user.roles.indexOf('admin') > -1
            return next()
          })
          .catch(() => {
            this.$log.warn('Unauthorized access with invalid/expired token')
            req.token = undefined
            req.user = {
              username: 'guest',
              fullName: 'Guest User',
