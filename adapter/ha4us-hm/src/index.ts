...
function Adapter(
    ...
    newDevices: async (err, params, callback) => {
      ...
      const newDevices = params[1].filter((device: any) => {
        if (
          ...
          if (IGNORE_TYPES.indexOf(device.TYPE) === -1) {
            $log.warn(
              `Device ${device.ADDRESS}-${device.TYPE} ignored. Parent=${
                device.PARENT
              } PARAMSETS=${device.PARAMSETS.join(',')}`
            )
          }
          ignored.push({
      ...
      $log.debug(
        `${newDevices.length} devices to be processed - ${
          ignored.length
        } will be ignored`
      )
  ...
  function regaYaml(script): any {
    ...
    return got
      ...
      .then(result => {
        const yml = iconv.decode(Buffer.from(result.body), 'ISO-8859-1')
  ...
  async function $onInit(): Promise<boolean> {
    ...
    if (typeof localAddress !== 'string' && !$args.hmHostIp) {
      ...
      throw new Ha4usError(
        500,
        `Can't determine host ip at ${$args.hmInterfaces}`
      )
    }
    ...
    $log.debug(
      'Using interfaces',
      interfaces.map(iface => iface.name).join(',')
    )
