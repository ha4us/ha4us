...
export abstract class AbstractVisorEntity
    ...
    ngOnInit() {
        ...
        this.sub.add(
            this.vs.editMode$.subscribe(mode => (this.editMode = mode))
        )
    ...
    public resizeEnd($event: ResizeEvent) {
        ...
        if (this.isResizing) {
            if ($event.edges.right) {
                changes.width = this.width + <number>$event.edges.right
            }
            if ($event.edges.left) {
                changes.width = this.width - <number>$event.edges.left
                changes.x = this.x + <number>$event.edges.left
            }
            if ($event.edges.bottom) {
                changes.height = this.height + <number>$event.edges.bottom
            }

            if ($event.edges.top) {
                changes.height = this.height - <number>$event.edges.top
                changes.y = this.y + <number>$event.edges.top
            }
