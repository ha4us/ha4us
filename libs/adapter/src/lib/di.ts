...
export class DIContainer {
  ...
  constructor() {
    if (DIContainer._instance) {
      throw new Error('singleton - please user DIContainer.getContainer()')
    }
  ...
  public registerFactory<T>(tag: string | symbol, factory: FactoryFunction<T>) {
  ...
  public registerService<T extends object>(
    tag: string | symbol,
    aClass: ServiceConstructor<T>
  ) {
    debug('registerClass', aClass, typeof aClass)
    this._container.register(tag, asClass(aClass).singleton())
  }
