...
test('On instantiation creates an index', async t => {
  ...
  t.log('Secret', us['_secret'])
  t.is(typeof us['_secret'], 'string')
})
...
test('Create an admin user', async t => {
  ...
  await t.throws(t.context.us.verify('admin', 'testpw'), /^401/)
})
...
test('Posting,putting and getting users', async t => {
  ...
  await t.throws(t.context.us.post(new Ha4usUser('testuser')), /^409/)
  ...
  return t.throws(t.context.us.put(usernew), /^404/)
})
...
test('Tokens', async t => {
  ...
  await t.throws(t.context.us.verify(token.token), /404/)

  clock.tick(1000 * 60 * 15 + 1)
  await t.throws(t.context.us.verify(token.token), /403/)
