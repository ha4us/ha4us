...
export class HMJson {
  ...
  constructor($log: Ha4usLogger, $args: any) {
    ...
    $log.debug('Logging in to HMJson as %s with %s', this.hmUser, this.hmPassword);
  ...
  protected async sendRequest(req) {
    ...
    return got(`http://${this.host}${JSON_URL}`, { body: req, json: true })
      .then((res) => {
        const body = res.body;
        if (body.error !== null) {
          throw new Ha4usError(body.error.code, body.error.name + '-' + body.error.message);
        } else {
          return body.result;
        }
      });
  ...
  async logout(): Promise<boolean> {
    ...
    if (this.sessionId !== null) {
      ...
      this.$log.debug('logging out from hmjson session %s ....', this.sessionId);
      return this.sendRequest({
        'version': '1.1',
