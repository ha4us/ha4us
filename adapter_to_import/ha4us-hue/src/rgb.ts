...
export type TColorGamut = [[number, number], [number, number], [number, number]];
...
export class XY2RGBConverter {
    ...
    public toRGB(xy: TXy, brightness: number): string {
        ...
        rgb = rgb.map(function (_x) {
            return (_x <= 0.0031308) ? (12.92 * _x) : ((1.0 + 0.055) * Math.pow(_x, (1.0 / 2.4)) - 0.055);
        });


        // Bring all negative components to zero.
        rgb = rgb.map(function (_x) { return Math.max(0, _x); });

        // If one component is greater than 1, weight components by that value.
        const max = Math.max(rgb[0], rgb[1], rgb[2]);
        if (max > 1) {
            rgb = rgb.map(function (_x) { return _x / max; });
        }

        rgb = rgb.map(function (_x) { return Math.floor(_x * 255); });
