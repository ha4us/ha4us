...
export  abstract class AlexaCapability {
  ...
  protected static createResponse (name: string, namespace: string, version: string): IAlexaResponse {
    return {
      event: {
        header: {
          name: name,
  ...
  constructor ($log: IHa4usLogger,  $db: Db, $states: StatesService) {
    ...
    async abstract handleRequest(req: IAlexaRequest): Promise<IAlexaResponse>

    async resolveToken (token: string): Promise<any> {

      const url = `https://api.amazon.com/user/profile?access_token=${encodeURIComponent(token)}`;
    ...
    protected getObjects(cookie: {room?: string, func?: string, name?: string}, role: string) {
      let dpCursor: Cursor;
      if (cookie.room && cookie.func) {
         ...
         dpCursor = this.db.find({name: cookie.name, role: role});
      }
